...
async function main() {
  ...
  try {
    ...
    const largestNumber = await page.evaluate(() => {
      const numbers = Array.from(document.querySelectorAll(".sc-eYqcxL")).map((el) => parseInt(el.textContent));
    ...
    for (let i = 1; i <= largestNumber; i++) {
      ...
      for (let x = 0; x < questionElement.length; x++) {
        ...
        if (isMCQ) {
          ...
          const text = await page.evaluate((el) => el.textContent, element);
          questionsArray.markScheme.push([text, questionsArray.question[x][1].replace("qp", "ms")]);
          await sleep(1000);
        }
        {
        }
